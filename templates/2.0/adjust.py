...
def str_to_version(v_str) -> (tuple[int, int] | None):
    """
...
def adjust(config, mapping) -> None:  # pylint: disable=unused-argument
    ...
    for component in config:
        try:
            config_version = str_to_version(config[component].get("version", "1.0"))
        ...
        if config_version != mapping_version:
            # Do not apply adjustments if the component version doesn't match
            # the mapping version
            continue

        if component in replace["components"].keys():
        ...
        if component in tolist["components"].keys():
