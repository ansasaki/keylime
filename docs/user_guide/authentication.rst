...
* **PoP (Proof of Possession) bearer tokens**: Used by agents for attestation operations
* **mTLS client certificates**: Used by administrators for management operations
...
To revoke an administrator's access:

1. Revoke their certificate using the CA:

   .. code-block:: bash

      keylime_ca -d /var/lib/keylime/cv_ca revoke -n admin1.example.com

2. Restart the verifier to clear any cached connections
...
**Cause**: An admin (mTLS) is trying to access an agent-only endpoint.
...
Verifying Certificate CN
^^^^^^^^^^^^^^^^^^^^^^^^^

To check the CN of a certificate:

.. code-block:: bash

   openssl x509 -in /var/lib/keylime/cv_ca/client-cert.crt -noout -subject
   # Output: subject=C=US, CN=client, ST=..., L=..., O=..., OU=...
