...
def adjust(config, mapping):
        ...
        for option in options.keys():
            ...
            for to_replace in values.keys():
                ...
                if config[component] and config[component][option] == to_replace:
                    ...
                    print(f"[{component}] In \"{option}\", replaced \"{to_replace}\" "
                          f"with \"{values[to_replace]}\"")
    ...
    for component in tolist["components"].keys():
        for option in tolist["components"][component]:
            ...
            try:
                ...
                print(f"[{component}] In option '{option}', failed to parse "
            ...
            print(f"[{component}] For option '{option}', converted '{value}' to "
                  f"'{config[component][option]}'")

    # Other special adjustments
    if config["verifier"]["tls_dir"] == "generate":
        for o in ["client_key", "client_cert", "trusted_server_ca",
                "server_key", "server_cert", "trusted_client_ca"]:
            if not config["verifier"][o] == "default":
                config["verifier"][o] = "default"
                print(f"[verifier] Replaced option '{o}' with 'default' as "
                      f"'tls_dir' is set as 'generate'")
    ...
    if config["registrar"]["tls_dir"] == "generate":
        for o in ["server_key", "server_cert", "trusted_client_ca"]:
            if not config["registrar"][o] == "default":
                ...
                print(f"[registrar] Replaced option '{o}' with 'default' as "
                      f"'tls_dir' is set as 'generate'")
    ...
    if config["agent"]["trusted_client_ca"] == "default":
        if config["verifier"]["tls_dir"] == "generate":
            ...
            print(f"[agent] For option 'trusted_client_ca', replaced 'default' with "
    ...
    if config["tenant"]["tls_dir"] == "default":
        if config["tenant"]["trusted_server_ca"] == "default":
            if config["registrar"]["tls_dir"] == "generate":
                if config["verifier"]["tls_dir"] == "generate":
                    config["tenant"]["trusted_server_ca"] = "['/var/lib/keylime/cv_ca/cacert.crt', '/var/lib/keylime/reg_ca/cacert.crt']" 
                    print(f"[tenant] For option 'trusted_server_ca', replaced 'default' with "
