...
# user.
CONFIG_FILES = ["/usr/etc/keylime.conf", "/etc/keylime.conf",
...
def get_config(config_files):
    ...
    if not any(os.path.exists(c) for c in config_files):
        print(
            f"Could not find any file of the files in {config_files}. "
            "Please set input file using --input option."
        )
        return None
    else:
...
def output(args, config, templates):
    """
    Output the requested files
    """

    flat = [s for ss in args.component for s in ss]
    ...
    for component in components:
        ...
        # Set output path
        o = os.path.join(args.out, f"{component}.conf")
...
def process_mapping(old_config, templates, mapping_file, debug=False):
    ...
    print(
        f"Applying mapping from file {mapping_file} version {mapping['version']}")
    ...
    if not os.path.isdir(version_dir):
        raise Exception(f"Could not find directory {version_dir} for version "
                        f"{mapping['version']} set in {mapping_file}")
    ...
    for component in mapping['components'].keys():
        ...
        for option in m.keys():
            ...
            try:
                ...
                print(
                    f"[{component}] {e} not found: Using default value \"{info['default']}\" for \"{option}\"")
    ...
    if os.path.isfile(adjust_script):
        try:
            ...
            spec = importlib.util.spec_from_file_location(
                "adjust", adjust_script)
            if not spec or not spec.loader:
                raise Exception(
                    f"Could not create spec to load {adjust_script}")
...
def process_versions(templates, old_config, target_version=None, debug=False):
    ...
    if target_version:
        ...
        if not target:
            raise Exception(f"Invalid target version number provided: "
                            f"{target_version}")

        if not target in versions:
            raise Exception(f"Directory for target version {target_version} not "
                            f"found in {templates}")
    ...
    for version in versions:
        ...
        if os.path.isdir(p):
            ...
            if os.path.isfile(m):
                ...
                if not old_version:
                    raise Exception(f"Invalid version number '{new['version']}'"
                                    f"in {m}")
            else:
...
def main():
    parser = argparse.ArgumentParser(description='Split keylime configuration'
                                     'file into individual files')

    parser.add_argument('--component', help='Select the components for which '
    ...
    parser.add_argument('--debug', help='Print resulting config after each '
                        'applied mapping in JSON format', default=False,
                        action='store_true')

    parser.add_argument('--input', help='Input keylime configuration file to '
    ...
    parser.add_argument('--out', help='Output directory where to put the '
                        'generated files. Default is current directory', default='.')

    parser.add_argument('--mapping', help='If provided, then only the provided '
    ...
    parser.add_argument('--templates', help='Path to the directory containing '
                        'the templates for the configuration files. If not '
                        'provided "./templates" is used', default='./templates')

    parser.add_argument('--version', help='Target version for the output '
    ...
    if args.version and not str_to_version(args.version):
        raise Exception(f"Invalid version {args.version} specified in --version: "
                        f"Expected 'MAJOR.MINOR' format")

    if not os.path.exists(args.templates):
        raise Exception(
            f"Templates directory {args.templates} does not exist")
    ...
    if args.mapping:
        if os.path.isfile(args.mapping):
            ...
            config = process_mapping(old_config, args.templates, mapping_file,
                    debug=args.debug)
        else:
        ...
        config = process_versions(
            args.templates, old_config, target_version=args.version, debug=args.debug)

    output(args, config, args.templates)
