...
class TestConfig(unittest.TestCase):
    ...
    def test_single_config(self):
        ...
        config.CONFIG_FILES = {
                "test": [os.path.join(CONFIG_DIR, "keylime-1.conf")]
        }
    ...
    def test_merge_config(self):
        ...
        config.CONFIG_FILES = {
            "agent": [os.path.join(CONFIG_DIR, "agent.conf")]
        }
        config.CONFIG_ENV = {"agent": ""}
        config.CONFIG_SNIPPETS_DIRS = {
            "agent": [os.path.join(CONFIG_DIR, "agent.conf.d")]
        }
    ...
    def test_reexport_function(self):
        ...
        config.CONFIG_ENV = {"test": ""}
        config.CONFIG_SNIPPETS_DIRS = {"test": ""}
        self.assertEqual(config.get("test", "attribute", section="default",
                                    fallback=""), "")
