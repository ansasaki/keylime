...
class TestAgentControllerCreate(unittest.TestCase):
    ...
    def test_create_v3_success_returns_201(
        self, mock_config, mock_build, mock_mtls, mock_ima_svc, mock_mb_svc, mock_agent_model, _mock_shared_mem
    ):
    ...
    @patch(f"{MODULE}.build_agent_data")
    @patch(f"{MODULE}.config")
    def test_create_v3_duplicate_returns_409(
        self, mock_config, mock_build, mock_mtls, mock_agent_model, _mock_shared_mem
    ):
...
class TestAgentControllerCreateRaceCondition(unittest.TestCase):
    """Test TOCTOU race condition handling in AgentController.create()."""
    ...
    @patch(f"{MODULE}.build_agent_data")
    @patch(f"{MODULE}.config")
    def test_create_v3_integrity_error_returns_409(
        self, mock_config, mock_build, mock_mtls, mock_ima_svc, mock_mb_svc, mock_agent_model, _mock_shared_mem
    ):
        """Test that database constraint violation during concurrent enrollment returns 409.

        Simulates the race where two concurrent requests both pass the duplicate
        check, but the second commit fails with IntegrityError due to the
        primary key constraint on agent_id.
        """
        controller = _v3_controller()
        controller._api_request_body = MagicMock()  # pylint: disable=protected-access
        controller.send_response = MagicMock()
        ...
        # Duplicate check passes (agent doesn't exist yet)
        mock_new_agent = MagicMock()
        mock_agent_model.get.return_value = None
        mock_agent_model.return_value = mock_new_agent

        # Simulate IntegrityError on commit (concurrent request already inserted)
        orig_exception = Exception("UNIQUE constraint failed: verifiermain.agent_id")
        mock_new_agent.commit_changes.side_effect = IntegrityError("INSERT INTO verifiermain ...", None, orig_exception)
