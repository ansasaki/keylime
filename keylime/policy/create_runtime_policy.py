...
def update_base_policy(base_policy_file: str) -> Optional[RuntimePolicyType]:
    ...
    try:
        ...
        policy["ima"]["log_hash_alg"] = base_policy.get("ima",
                                                        {}).get("log_hash_alg",
                                                                DEFAULT_IMA_TEMPLATE_DIGEST_ALGORITHM)
...
def _get_digest_algorithm_from_map_list(maplist: Dict[str, List[str]]) -> str:
    ...
    algo = "unknown"
    if maplist:
        _, digest_list = next(iter(maplist.values()))
        if digest_list:
...
def create_runtime_policy(args: argparse.Namespace) -> Optional[RuntimePolicyType]:
    ...
    ima_digests = None
    rootfs_digests = None
    ramdisk_digests = None
    local_rpm_digests = None
    remote_rpm_digests = None
    ...
    if args.base_policy:
        # TODO: obtain algorithm used in base policy
    ...
    if args.use_measurement_list:
        try:
            ...
            ima_measurement_list_algo = "unknown"
    ...
    # priority order: base policy > allowlist > IMA measurement list > --algo
    for a, source in [
    ...
    for digests in [ima_digests, rootfs_digests, ramdisk_digests, local_rpm_digests, remote_rpm_digests]:
        if not digests:
            continue
