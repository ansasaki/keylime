...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # If agent pubkey requested
            ...
            quote = tpm_instance.create_quote(nonce, self.server.public_key_exportable, pcrmask, hash_alg)
            imaMask = pcrmask
            ...
            if "partial" in rest_params and (rest_params["partial"] is None or rest_params["partial"] == "1"):
                ...
                response = {
                    ...
                    "enc_alg": enc_alg,
                    "sign_alg": sign_alg,
                    "pubkey": self.server.public_key_exportable,
                }
...
class CloudAgentHTTPServer(ThreadingMixIn, HTTPServer):
    ...
    def __init__(self, server_address, RequestHandlerClass, agent_uuid, contact_ip, ima_log_file, tpm_log_file_data):
        ...
        (cert_path, key_path, trusted_ca, key_password), verify_client = web_util.get_tls_options("agent", logger=logger)
        ...
        if self.mtls_cert_enabled:
            if os.path.isfile(cert_path):
                ...
                with open(cert_path, "wb") as f:
                    ...
                    cert = crypto.generate_selfsigned_cert(agent_uuid,
                            private_key, valid_util, agent_ips)
...
def main():
    ...
    serveraddr = (config.get("agent", "ip"),
    ...
    if server.mtls_cert_enabled:
        context = web_util.generate_tls_context(server.cert_path,
            server.private_key_path, server.trusted_ca,
            private_key_password=server.private_key_password, logger=logger)
