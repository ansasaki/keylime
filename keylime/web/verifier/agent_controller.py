...
class AgentController(Controller):
    ...
    def _create_v3(self, agent: Optional[Dict[str, Any]] = None, **_params: Any) -> None:
        ...
        agent_locks = shared_mem.get_or_create_dict("verifier_agent_enrollment_locks")
        ...
        with agent_lock:
            ...
            try:
                ...
                logger.warning("Agent enrollment failed due to database constraint for '%s': %s", agent_id, e)
                APIError("conflict").set_detail(  # type: ignore[no-untyped-call]
                    f"Agent '{agent_id}' already exists. Use DELETE then POST to re-enroll."
                ).send_via(self)
                return
