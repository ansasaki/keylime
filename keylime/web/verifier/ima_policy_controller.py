...
def _validate_and_format_ima_policy(
    ...
    try:
        db_format = ima.runtime_policy_db_contents(name, runtime_policy, tpm_policy or "")
    except ima.ImaValidationError as e:
...
class IMAPolicyController(Controller):
    ...
    def _index_v3(self) -> None:
        ...
        data = [
            APIResource(
                "ima_policy",
                str(policy.name),  # type: ignore[attr-defined]
                _render_ima_policy_attrs(policy),  # type: ignore[arg-type]
            )
            .include(APILink("self", f"/v{self.version}/policies/ima/{policy.name}"))  # type: ignore[attr-defined]
    ...
    def _update_v3(self, name: str, ima_policy: Optional[Dict[str, Any]] = None, **_params: Any) -> None:
        ...
        APIResource(
            "ima_policy",
            str(name),
            _render_ima_policy_attrs(existing),  # type: ignore[arg-type]
        ).include(APILink("self", f"/v{self.version}/policies/ima/{name}")).send_via(
            self, code=200
        )  # type: ignore[no-untyped-call]
    ...
    def _delete_v3(self, name: str) -> None:
        ...
        agents = VerifierAgent.all_ids(ima_policy_id=policy.id)  # type: ignore[no-untyped-call, attr-defined]
        if agents:
            APIError("conflict").set_detail(  # type: ignore[no-untyped-call]
