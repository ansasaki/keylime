...
def doRegisterAgent(
    registrar_ip, registrar_port, agent_id, ek_tpm, ekcert, aik_tpm,
    mtls_cert=None, contact_ip=None, contact_port=None
):
    """
    ...
    try:
        ...
        client = RequestsClient(f"{registrar_ip}:{registrar_port}", False, ignore_hostname=True)
...
def doRegistrarDelete(registrar_ip, registrar_port, agent_id, tls_context):
    ...
    client = RequestsClient(f"{registrar_ip}:{registrar_port}", True,
            tls_context=tls_context,
            ignore_hostname=True)
...
def doRegistrarList(registrar_ip, registrar_port, tls_context):
    ...
    """
    client = RequestsClient(f"{registrar_ip}:{registrar_port}", True,
            tls_context=tls_context, ignore_hostname=True)
