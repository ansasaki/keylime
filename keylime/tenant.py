...
class Tenant:
    ...
    def __init__(self):
        ...
        (cert, key, trusted_ca, key_password), verify_server_cert = web_util.get_tls_options("tenant", is_client=True, logger=logger)
            ...
            if not verify_server_cert:
                ...
                if not trusted_ca:
                    ...
                    self.tls_context = web_util.generate_tls_context(
                            cert, key, trusted_ca, key_password,
                            verify_server_cert, is_client=True, logger=logger)
    ...
    def init_add(self, args):
        ...
        self.registrar_data = registrar_client.getData(self.registrar_ip,
                                                       self.registrar_port,
                                                       self.agent_uuid,
                                                       self.tls_context)
        ...
        if self.payload is not None:
            ...
            if len(self.payload) > max_payload_size:
                raise UserError(
                    f"Payload size {len(self.payload)} exceeds max size {max_payload_size}"
                )
    ...
    def do_cv(self):
        ...
        do_cv = RequestsClient(self.verifier_base_url, True,
                tls_context=self.tls_context, ignore_hostname=True)
        response = do_cv.post(
    ...
    def do_cvstatus(self):
        ...
        do_cvstatus = RequestsClient(self.verifier_base_url, True,
                tls_context=self.tls_context, ignore_hostname=True)
    ...
    def do_cvdelete(self, verifier_check=True):
        ...
        do_cvdelete = RequestsClient(self.verifier_base_url, True,
                tls_context=self.tls_context, ignore_hostname=True)
        ...
        if response["code"] == 202:
            deleted = False
            for _ in range(12):
                get_cvdelete = RequestsClient(self.verifier_base_url, True,
                        tls_context=self.tls_context, ignore_hostname=True)
    ...
    def do_regstatus(self):
        agent_info = registrar_client.getData(self.registrar_ip, self.registrar_port,
                                            self.agent_uuid,
                                            self.tls_context)
    ...
    def do_reglist(self):
        ...
        response = registrar_client.doRegistrarList(self.registrar_ip,
                                                    self.registrar_port,
    ...
    def do_cvreactivate(self, verifier_check=True):
        ...
        do_cvreactivate = RequestsClient(self.verifier_base_url, True,
                tls_context=self.tls_context, ignore_hostname=True)
    ...
    def do_cvstop(self):
        ...
        do_cvstop = RequestsClient(self.verifier_base_url, True,
                tls_context=self.tls_context, ignore_hostname=True)
    ...
    def do_add_allowlist(self, args):
        ...
        cv_client = RequestsClient(self.verifier_base_url, True,
                tls_context=self.tls_context, ignore_hostname=True)
    ...
    def do_show_allowlist(self, name):
        cv_client = RequestsClient(self.verifier_base_url, True,
                tls_context=self.tls_context, ignore_hostname=True)
        response = cv_client.get("/v{self.api_version}/allowlists/{name}")
