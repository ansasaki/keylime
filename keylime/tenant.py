...
class Tenant:
    ...
    def _negotiate_server_version(
        self, server_versions: Optional[List[str]], server_name: str, local_versions: Optional[List[str]] = None
    ) -> str:
        ...
        Args:
            ...
            local_versions: List of versions supported locally, or None for all versions
        ...
        try:
            ...
            assert negotiated is not None  # For type checker - raise_on_error=True guarantees non-None
    ...
    def negotiate_verifier_version(self) -> None:
        ...
        self.verifier_api_version = self._negotiate_server_version(server_versions, "verifier", local_versions)
    ...
    def negotiate_registrar_version(self) -> None:
        ...
        if self.push_model:
            ...
            local_versions = [v for v in keylime_api_version.all_versions() if keylime_api_version.major(v) < 3]
        self.registrar_api_version = self._negotiate_server_version(server_versions, "registrar", local_versions)
    ...
    def ensure_verifier_version(self) -> str:
        ...
        assert self.verifier_api_version is not None
    ...
    def ensure_registrar_version(self) -> str:
        ...
        assert self.registrar_api_version is not None
    ...
    def init_add(self, args: Dict[str, Any]) -> None:
        ...
        if (self.agent_ip is None or self.agent_port is None) and self.registrar_data is not None:
            if self.agent_ip is None and self.registrar_data["ip"] is not None:
        ...
        # Skip in push-mode since agent will send attestations to verifier directly
            ...
            logger.debug(
                "Attempting to contact agent at %s:%s to get API version (push_model=%s)",
                self.agent_ip,
                self.agent_port,
                self.push_model,
            )
    ...
    def do_cvadd(self) -> None:
        ...
        # Send ip/port from command line args (if provided)
        # The verifier will determine whether to use them based on its mode config:
        # - Push mode: verifier ignores ip/port and stores None
        # - Pull mode: verifier uses ip/port to poll the agent
