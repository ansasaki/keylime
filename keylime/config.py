    ...
    * Find and apply any local user override from files in
    ...
    if component not in _config:
        ...
        if CONFIG_ENV[component]:
            if os.path.exists(CONFIG_ENV[component]):
                if os.path.isfile(CONFIG_ENV[component]):
                    ...
                    print(f"Configuration file {CONFIG_ENV[component]} for"
        ...
        if not any(os.path.exists(c) for c in CONFIG_FILES[component]):
            print(
                f"Config file not found in {CONFIG_FILES[component]}. "
                f"Please see {__file__} for more details."
            )
            if _CONFIG_FALLBACK:
            ...
            for c in CONFIG_FILES[component]:
                ...
                if config_file:
                    ...
                    for d in (x for x in CONFIG_SNIPPETS_DIRS[component] if os.path.exists(x)):
                        snippets = sorted(x for x in filter(lambda v: os.path.isfile(v),
                                                            (os.path.join(d, f) for f in os.listdir(d) if f)))
...
def getlist(component, option, section=None, fallback=[]):
    if not section:
        section = component
    ...
    if read:
        try:
            ...
            if isinstance(l, list):
                ...
                raise Exception(f"Config option '{option}' in section '{section}' "
                                f"'of component {component} should be a list")
        except Exception as e:
            raise Exception(f"Failed to get list from config for component "
                            f"'{component}', section '{section}', option '{option}'")
    else:
        return fallback


def getdict(component, option, section=None, fallback={}):
    if not section:
        section = component
    ...
    if read:
        try:
            ...
            if isinstance(d, dict):
                ...
                raise Exception(f"Config option '{option}' in section '{section}' "
                                f"'of component {component} should be a dictionary")
        except Exception as e:
            raise Exception(f"Failed to get dictionary from config for component "
                            f"{component}, section {section}, option {option}")
    else:
        return fallback
