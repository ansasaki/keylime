...
def get_tls_dir(component):
    ...
    elif tls_dir == "generate":
        if component == "verifier":
            generatedir = "cv_ca"
        elif component == "registrar":
            generatedir = "reg_ca"
        else:
            raise Exception(f"The tls_dir=generate option is not supported for "
                            f"'{component}'")

        tls_dir = os.path.abspath(os.path.join(config.WORK_DIR, generatedir))
    elif tls_dir == "default":
        if (component == "verifier" or
           component == "registrar" or
           component == "tenant"):
    ...
    if tls_dir == "generate":
        if component == "verifier":
            ...
            options = ["server_cert", "server_key", "trusted_client_ca",
                    "client_cert", "client_key", "trusted_server_ca"]
        elif component == "registrar":
            generatedir = "reg_ca"
            options = ["server_cert", "server_key", "trusted_client_ca"]
        else:
        ...
        for option in options:
            ...
            if value != "default":
                raise Exception(
                    f"To use tls_dir=generate, the following options must be "
                    f"set to 'default': {options}"
                )
    ...
    try:
        ...
        if verify_peer_cert:
            if not trusted_ca and not ca_cert_string:
                if logger:
                    logger.error("Peer certificate verification is enabled, but no "
                                 "CA certificate was provided")
                    raise Exception("Peer certificate verification is enabled, but no "
                                    "CA certificate was provided")
            ...
            if ca_cert_string:
                temp_dir = tempfile.TemporaryDirectory(prefix="keylime_")
                temp_file = os.path.join(temp_dir.name, "agent.crt")
...
def generate_agent_tls_context(component, cert_blob, logger=None):
    ...
    if not verify_server:
        if logger:
            logger.warning("'enable_agent_mtls' is 'True', but"
                "'trusted_server_ca' is set as 'all', which disables server"
